...
class GoodIDAppUserChecker implements ClaimCheckerInterface
{
    /**
    ...
    public function checkClaim(JWTInterface $jwt)
    {
        ...
        $user = $jwt->hasClaim('user') ? $jwt->getClaim('user') : null;

        if ($acr < 3) {
            if ($user !== null) {
                throw new \InvalidArgumentException('Unverifiable user claim');
            }
            return [];
        }

        if ($user === null) {
            throw new \InvalidArgumentException('Missing user claim');
        }

        if (!$this->appSignatureChecklist->isClaimSigned('user')) {
            throw new \InvalidArgumentException('Unverifiable user claim');
        }
